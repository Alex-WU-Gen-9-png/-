...
pub export fn main() c_int {
    srand(@as(c_uint, @bitCast(@as(c_int, @truncate(time(null))))));
    var target: c_int = @import("std").zig.c_translation.signedRemainder(rand(), @as(c_int, 99)) + @as(c_int, 1);
    _ = &target;
    var guess: c_int = undefined;
    _ = &guess;
    var attempts: c_int = 0;
    _ = &attempts;
    _ = printf("Guess an integer between 1 and 100 (100 is not included):\n");
    while (true) {
        attempts += 1;
        if (scanf("%d", &guess) != @as(c_int, 1)) {
            _ = printf("Please enter a valid number: ");
            while (getchar() != @as(c_int, '\n')) {}
            continue;
        }
        if (guess < target) {
            _ = printf("Too small! Try again: ");
        } else if (guess > target) {
            _ = printf("Too big! Try again: ");
        } else {
            _ = printf("You win! You tried %d times.\n", attempts);
            break;
        }
    }
    return 0;
...
pub const __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 260001, .decimal);
pub const __ENVIRONMENT_OS_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 260001, .decimal);
...
// /Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include/malloc/_malloc_type.h:148:9
